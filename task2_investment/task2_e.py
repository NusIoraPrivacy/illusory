...
def get_combined_dialogue(statements_group, mode="direct", model_name="", cot=0):
    statements_group = statements_group.upper()
    assert statements_group in ['A', 'B']
    ...
    if cot > 0:
        footer += " Let's think step by step."
...
def run_task(model="Qwen2.5-14B-Instruct", temperature=1, n_cycles=100, mode="direct", max_new_tokens=1024, attribution_method="integrated_gradients", role="none", cot=0):
    ...
    for _ in range(repeat_times):
        combined_dialogue_a = get_combined_dialogue('A', mode, model, cot)
        combined_dialogue_b = get_combined_dialogue('B', mode, model, cot)
