...
prompt_decision_guidelines = (
    ...
    "Process:\n"
    "1. Label each statement as POSITIVE (helpful/kind/responsible) or NEGATIVE (harmful/careless/irresponsible)\n"
    "2. Calculate proportion for each group: positive_count / (positive_count + negative_count)\n"
    "3. Choose the group with the HIGHER proportion\n\n"
    "Example: Group A (4 positive, 1 negative = 80%) equals Group B (16 positive, 4 negative = 80%). "
    "Total statement counts are IRRELEVANT - only proportions matter.\n\n"
)

prompt_header = prompt_decision_guidelines + "\n\nNow please read the following descriptions:"
...
def run_task(model="Qwen2.5-14B-Instruct", temperature=1, n_cycles=100, mode="direct", max_new_tokens=1024, attribution_method="integrated_gradients"):
    ...
    with open(file_path, "w", encoding="utf-8") as f, open(explain_path, "w", encoding="utf-8") as fexp:
        ...
        print(f"Starting {len(all_cycles)} cycles...", flush=True)
        for i, (current_prompts, majority_group) in enumerate(all_cycles):
                    ...
                    try:
                        ...
                        # Open-source
                            if hasattr(tokenizer, "chat_template") and tokenizer.chat_template:
                                ...
                                gen_kwargs = dict(max_new_tokens=max_new_tokens, do_sample=True, temperature=temperature, top_p=0.95)
                                print("No Chat Template", flush=True)
                            print(f"Calling text_generator for cycle {i+1}...", flush=True)
                            outputs = text_generator(prompt_text, **gen_kwargs)
                            print(f"text_generator completed for cycle {i+1}", flush=True)
                ...
                if cycle_success and model not in ["gpt-4-turbo", "gpt-4o", "GPT-3.5", "GPT-4"] and attribution_method != "none":
                    ...
                    if attribution_method == "integrated_gradients":
                        ...
                        html_snippet, filtered_tokens, rank_scores = attention_rollout(model_hf, tokenizer, full_dialogue_text, out_prefix, mode="text", device=device, model_type=model_type, model_name=model_name)
