...
from utils.globals import OPENAI_API_KEY, OPENAI_API_BASE, gpt_list, all_model_list, roles1
...
To what extent do you think your coworker may be connected to you not getting the promotion (1 = not at all; 2 = very unlikely; 3 = somewhat unlikely; 4 = uncertain; 5 = somewhat likely; 6 = very likely; 7 = a great deal)? Provide **only** a number between 1 and 7."""
...
To what extent do you think the visits to the bed and breakfast may be connected to the earnings you made off your stocks (1 = not at all; 2 = very unlikely; 3 = somewhat unlikely; 4 = uncertain; 5 = somewhat likely; 6 = very likely; 7 = a great deal)? Provide **only** your choice ("1", "2", "3", "4", "5", "6", "7")."""
prompt5_direct = """Now, please read Task 1: Imagine that you buy stock in one of the three construction companies that service your area. One day, your spouse, who runs the local bed and breakfast, notes that the families of all three company owners have checked into the B&B recently. Later, the prices all three companies offer for their services have risen drastically. 
Because of the higher prices, all three companies post very high profits, and you make a lot of money off of the stock you own. 
To what extent do you think the visits to the bed and breakfast may be connected to the earnings you made off your stocks (1 = not at all; 2 = very unlikely; 3 = somewhat unlikely; 4 = uncertain; 5 = somewhat likely; 6 = very likely; 7 = a great deal)? Provide **only** a number between 1 and 7."""
...
Because of the higher prices, all three companies post very high profits, and you make a lot of money off of the stock you own. 
To what extent do you think the visits to the bed and breakfast may be connected to the earnings you made off your stocks (1 = not at all; 2 = very unlikely; 3 = somewhat unlikely; 4 = uncertain; 5 = somewhat likely; 6 = very likely; 7 = a great deal)?"""
...
def run_task(model="Qwen2.5-14B-Instruct", temperature=1, n_cycles=100, mode="direct", max_new_tokens=1024, attribution_method="integrated_gradients", role="none", cot=0):
    if RANDOM_STATE:
    ...
    if mode == "direct":
        ...
        global prompt3_explain, prompt4_explain, prompt5_explain, prompt6_explain
        if cot > 0:
            prompt3_explain += " Let's think step by step."
            prompt4_explain += " Let's think step by step."
            prompt5_explain += " Let's think step by step."
            prompt6_explain += " Let's think step by step."
    ...
    file_name = f"model_{model_name}_mode_{mode}_temperature_{temperature}_cycles_{n_cycles}_role_{role}_cot_{cot}.txt"
    ...
    with open(file_path, "w", encoding="utf-8") as f, open(explain_path, "w", encoding="utf-8") as fexp:
        ...
        for i, current_prompts in enumerate(all_cycles):
            ...
            if role == "none":
                ...
                messages = [{'role': 'system', 'content': f"I want you to act as a {role} participant."}]
            else:
                messages = [{'role': 'system', 'content': f"I want you to act as a decision maker with an {role} school education."}]
                ...
                try:
                    ...
                    # Open-source
                        if hasattr(tokenizer, "chat_template") and tokenizer.chat_template:
                            ...
                            # Filter out system messages and merge with first user message if needed
                            filtered_messages = []
                            i = 0
                            while i < len(messages):
                                msg = messages[i]
                                if msg["role"] == "system":
                                    # If next message is user, merge system content into it
                                    if i + 1 < len(messages) and messages[i + 1]["role"] == "user":
                                        next_msg = messages[i + 1]
                                        filtered_messages.append({"role": "user", "content": f"{msg['content']}\n\n{next_msg['content']}"})
                                        i += 2  # Skip both system and user messages
                                        continue
                                    else:
                                ...
                                i += 1
                            prompt_text = tokenizer.apply_chat_template(filtered_messages, tokenize=False, add_generation_prompt=True)
                            gen_kwargs = dict(max_new_tokens=max_new_tokens, do_sample=True, temperature=temperature, top_p=0.95)
                            print("Chat Template (system role handled)")
                        else:
...
def main():
    ...
    run_task(args.model, args.temperature, args.n_cycles, args.mode, args.max_new_tokens, args.attribution_method, args.role, args.cot)
